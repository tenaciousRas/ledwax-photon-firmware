...
#define NUM_LEDS_SPARKFUN_WS2801_1METER 5
...
uint8_t pwmStripPins[NUM_STRIPS][NUM_LEDS_PWM_RGB_STRIP] = { { 0, 1, 2 }, { } };
...
uint32_t ledColorOld[NUM_STRIPS][NUM_LEDS_SPARKFUN_WS2801_1METER] = { { RED, OFF }, { TWYELLOW, TWBLUE } }; // color before fade
...
//	Serial.print(F("startup\n"));
    ...
    Particle.variable(
            "stripStateJSON", &stripStateJSON, STRING);
#ifdef DEBUG_MODE
...
int setLEDStripColor(string command) {
    char data[40];
    // expect to have Int comma Int (int,int) from apps
    uint8_t b1[2];
    uint8_t b2[38];
    b1[0] = command[0];
    b1[1] = '\0';
...
/**
 * Calculate the fade colors for all pixels of the given strip.
 */
...
/**
 * Makes the whole strip a single color that cycles through
 * a rainbow of colors.
 */
...
/**
 * Render pixels using PWM or SPI.
 */
        ...
        case STRIP_TYPE_PWM:
            for (int i = 0; i < stripNumPixels[stripNum]; i++) {
                for (int j = 0; j < stripNumColorsPerPixel[stripNum]; j++) {
                    ...
                    pwmDriver.setPin(
                            pwmStripPins[stripNum][j], (uint16_t) ledColorFadeToChannels[j], false);
                }
            }
            break;
