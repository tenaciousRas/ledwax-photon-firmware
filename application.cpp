...
#endif
  // send init data
  Spark.function(setRemoteControlStripIndex, 'stripidx');
  Spark.function(setLEDStripColor, 'stripcolor');
  Spark.function(setDispMode, 'dispmod');
  Spark.function(setBright, 'bright');
  Spark.function(setLedFadeMode, 'fademode');
  Spark.function(setLedFadeTimeInterval, 'fadetimeint');
  Spark.function(setMultiColorHoldTime, 'colorholdtime');
  Spark.send("led controller connected via WiFi.\n");
#ifdef DEBUG_MODE
...
void setRemoteControlStripIndex(String command) {
  int bData;
...
void setLEDStripColor(String command) {
  char data[40];
...
void setDispMode(String command) {
  int bData;
  bData = atoi(command.c_str());
...
void setLedFadeTimeInterval(String command) {
  unsigned long bData;
...
void setMultiColorHoldTime(String command) {
  unsigned long bData;
  bData = atol(command.c_str());
...
void setLedFadeMode(String command) {
  unsigned int bData;
  bData = atoi(command.c_str());
