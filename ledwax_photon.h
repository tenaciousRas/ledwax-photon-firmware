...
public:
	...
	LEDWaxPhoton(uint8_t stripType[], uint8_t stripNumPixels[],
	...
	// METHOD DECLARATIONS
	void begin(), renderAll(), defaultStripState(uint8_t), readStripState(
			led_strip_disp_state*), putStripState(led_strip_disp_state*),
			setDispModeColors(uint8_t, int), refreshLEDs(uint8_t), turnOffLEDs(
					uint8_t), white(uint8_t), solidMultiColor(uint8_t, int),
			alternatingMultiColor(uint8_t, int), solidOneColor(uint8_t),
			solidTwoColors(uint8_t), solidThreeColors(uint8_t),
			alternatingTwoColors(uint8_t), alternatingTwoRandomColors(uint8_t),
			alternatingThreeColors(uint8_t), startFade(uint8_t), doFade(
					uint8_t), randomCandy(uint8_t), rainbow(uint8_t, uint16_t),
			rainbowCycle(uint8_t, uint16_t), colorWipe(uint8_t, uint8_t),
			renderPixels(uint8_t);
	string
	buildStripStateJSON();
	int setRemoteControlStripIndex(string),
			setLEDStripColor(string), setDispMode(string), setBright(string),
			setLedFadeTimeInterval(string), setMultiColorHoldTime(string),
			setLedFadeMode(string);
	uint32_t rgbColor(uint8_t, uint8_t, uint8_t), wheel(uint8_t);

private:
	const uint8_t stripType[], stripNumPixels[], // 1 for PWM strip
	...
	Adafruit_NeoPixel addressableStrip[];
...
// };
	...
	uint32_t ledColorOld[NUM_STRIPS][NUM_LEDS_SPARKFUN_WS2801_1METER] = { { RED,
